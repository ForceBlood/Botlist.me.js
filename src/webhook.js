...
class BotlistMeWebhook extends EventEmitter {
  /**
   * Creates a new BotlistMeWebhook Instance.
   * @param {number} port The port to run the webhook on.
   * @param {string} [path='/botlistmewebhook'] The path for the webhook request.
   * @param {string} [auth] The string for Authorization you set on the bot page for verification.
  ...
  constructor(port, path, auth, server) {
    ...
    super();
    this.port = port || 0;
    this.path = path || '/botlistmewebhook';
    this.auth = auth;
...
module.exports = BotlistMeWebhook;